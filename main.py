...
def main():    
    # Original maps
    original_small_map = TestModel(num_episodes=15, map=SmallMap, visual_radius=5, warmup_steps=1000)
    ...
    gifting_open_map = TestModel(num_episodes=15, map=OpenMap, visual_radius=5, warmup_steps=1000, action_policy=ActionPolicy.GIFT_ONLY)
    gifting_open_map.execute()

    # Agents with different policies and actions
    agents_with_different_policies = TestModel(num_episodes=15, map=OpenMap, visual_radius=5, warmup_steps=1000, action_policy=ActionPolicy.MIXED)
